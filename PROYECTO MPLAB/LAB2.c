...
//    ADCON0bits.GO = 1;                              // Comenzar conversión ADC
    ...
    InitLCD(2,11);                          // Se inicia la LCD con 2 líneas y caracteres de 11 pixeles de alto 
    MoveCursor(1,0,1);                      // Línea 1. Desplazar 0 posiciones a la derecha
    WriteCharLCD("Eduardo Andres");
    MoveCursor(2,1,1);                      // Línea 2. Desplazar 1 posición a la derecha
    WriteCharLCD("Santizo Olivet");
    __delay_ms(5000);                       // Delay para permitir que se lea el nombre
    
    ResetLCD();                             // Se limpia la pantalla y el cursor se regresa a la primera línea
